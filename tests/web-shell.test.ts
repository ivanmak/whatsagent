...
test("renderWebShell emits parseable dashboard JavaScript", () => {
  ...
  expect(script).toContain("const suffix = '/messages?limit=' + MESSAGE_HISTORY_PAGE_SIZE");
  expect(script).toContain("if (beforeId > 0) void loadMessages({ beforeId, rerender: true, silent: true, scrollMode: 'preserve' });");
  ...
  expect(messagesSource).toContain("channelOlderExhausted");
...
test("EP-015 WA-059: Settings About uses identity card", () => {
  ...
  expect(settingsSource).toContain("class=\"about-changelog-entry\">");
  expect(settingsSource).not.toContain("(index === 0 ? ' open' : '')");
  ...
  expect(settingsSource).not.toContain("View full history");
