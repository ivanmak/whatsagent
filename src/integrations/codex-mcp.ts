...
export interface CodexToolHandlers {
...
export const CODEX_TOOL_NAMES = ["whoami", "list_peers", "list_kanban_tasks", "read_kanban_task", "create_kanban_task", "update_kanban_task", "update_kanban_task_status", "comment_kanban_task", "archive_kanban_task", "list_kanban_epics", "read_kanban_epic", "create_kanban_epic", "update_kanban_epic", "comment_kanban_epic", "archive_kanban_epic", "update_kanban_epic_status", "request_kanban_epic_close", "cancel_kanban_epic_close", "send_message", "broadcast_message", "post_channel_message", "reply_channel_thread", "read_channel_messages", "search_direct_messages", "search_channel_messages", "search_kanban_tasks", "search_kanban_epics", "check_messages", "set_summary"] as const;

export function createCodexToolHandlers(context: LaunchContext, fetchImpl: FetchLike = fetch): CodexToolHandlers {
  return codexHandlersFromAgentTools(createAgentTools(context, fetchImpl));
}

export async function createValidatedCodexToolHandlers(env: Record<string, string | undefined> = process.env, fetchImpl: FetchLike = fetch): Promise<CodexToolHandlers> {
  ...
  return createCodexToolHandlers(context, fetchImpl);
}

export function createCodexMcpServer(context: LaunchContext, fetchImpl: FetchLike = fetch, agentText: AgentTextSettings = DEFAULT_AGENT_TEXT_SETTINGS, rbac: RbacBootSnapshot = PERMISSIVE_RBAC_BOOT_SNAPSHOT): McpServer {
  const handlers = createCodexToolHandlers(context, fetchImpl);
  const server = new McpServer(
    { name: "whatsagent", version: "0.1.0" },
    {
      instructions: `${agentText.colleagueProtocol}

DELIVERY ON THIS SIDE (Codex):

Codex does not currently have a WhatsAgent native push channel. Live messages
arrive by a short PTY nudge asking you to call check_messages. Treat that nudge
as a signal only; the full authoritative message body is delivered by the
check_messages tool as a WHATSAGENT INBOX block.
...
answering or changing files. If WhatsAgent asks you to call check_messages,
do that before continuing. Reply only when substantive; do not auto-acknowledge.`,
    },
  );

  // EP-022 / WA-097: register-time visibility filter, mirrors claude-mcp.ts.
  ...
  register("list_kanban_tasks", {
    ...
    inputSchema: {
      ...
      epicId: z.string().optional().describe("Filter by epic display id, or 'none' for unclassified tasks."),
    },
  ...
  register("read_kanban_task", {
    ...
    inputSchema: { taskId: z.string().describe("Task display id, for example WA-001.") },
  ...
  register("create_kanban_task", {
    ...
    inputSchema: {
      ...
      epicId: z.string().optional().describe("Optional parent epic display id."),
    },
  ...
  register("update_kanban_task", {
    ...
    inputSchema: {
      ...
      epicId: z.string().nullable().optional().describe("Parent epic display id; null to unlink; omit to leave unchanged."),
    },
  ...
  register("update_kanban_task_status", {
    ...
    inputSchema: {
      taskId: z.string(),
      status: z.enum(["Backlog", "Queued", "In Progress", "Blocked", "Review", "Completed"]),
    },
  ...
  register("check_messages", {
    ...
    description: "Deliver pending WhatsAgent messages. Call when nudged and first on every user turn before answering or editing files.",
    inputSchema: {
...
export async function runCodexMcpServer(env: Record<string, string | undefined> = process.env, fetchImpl: FetchLike = fetch): Promise<void> {
  const context = requireLaunchContext(env);
  if (!await validateLaunchContext(context, fetchImpl)) throw new Error("WhatsAgent launch token validation failed.");
  const agentText = await loadAgentTextSettings(context, fetchImpl);
  // EP-022 / WA-097: fetch RBAC visibility snapshot once at boot
  // (mirrors claude-mcp.ts).
  const rbac = await loadRbacBootSnapshot(context, fetchImpl);
  const server = createCodexMcpServer(context, fetchImpl, agentText, rbac);
...
function codexHandlersFromAgentTools(tools: AgentTools): CodexToolHandlers {
  return {
...
if (import.meta.main) await runCodexMcpServer();
