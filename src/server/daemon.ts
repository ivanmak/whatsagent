...
async function listFleetChannelMessages(state: DaemonState, ws: WorkspaceState, url: URL): Promise<Response> {
  const explicitRootLimit = url.searchParams.get("rootLimit");
  const rootLimit = tryParseInteger(explicitRootLimit ?? url.searchParams.get("limit"), { min: 1, max: explicitRootLimit == null ? 500 : 100, default: 20 });
  if (rootLimit instanceof Response) return rootLimit;
  const rootBeforeId = tryParseInteger(url.searchParams.get("rootBeforeId") ?? url.searchParams.get("beforeId"), { min: 0, default: 0 });
